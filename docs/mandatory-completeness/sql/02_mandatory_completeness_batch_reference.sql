--------------------------------------------------------------------------------
-- Referencia SQL del cálculo Mandatory usado por MandatoryCompletenessService.
-- Sustituye la TT ad-hoc original por la tabla reusable y un RUN_ID.
--------------------------------------------------------------------------------

-- :RUN_ID debe existir previamente en:
-- P360_EXPLOIT.TT_PRODUCT_COMPLETENESS_WORK(RUN_ID, PRODUCT_ID)
...
mandatory_config as (
    select /*+ materialize */ distinct
           mr."Template", mr."CharacteristicID", mr."Characteristic"
      from metadata_rows mr
     where mr."Property" = N'IsMandatory'
       and lower(trim(mr."PropertyValue")) in (N'1',N'true',N'yes',N'si',N'sí')
),
business_config as (
    select /*+ materialize */ distinct
           mr."Template", mr."CharacteristicID", mr."Characteristic",
...
business_scope as (
    select /*+ materialize */
           ...
           max(case
                 ...
                 then 1 else 0
               end) as "HasBusinessSpecificConfig"
      from products p
      left join business_config bc on bc."Template" = p."Template"
     group by p."InputIdentifier"
),
mandatory_applicable as (
    select /*+ materialize */ distinct
           p."InputIdentifier", p."ArticleRevisionID", p."Template",
           p."BusinessCode", p."BusinessName",
           mc."CharacteristicID", mc."Characteristic"
      from products p
      join business_scope bs on bs."InputIdentifier" = p."InputIdentifier"
      join mandatory_config mc on mc."Template" = p."Template"
...
mandatory_evaluation as (
    select /*+
           ...
           */
           ...
           ma."Characteristic",
           max(case
                 when acv."Value" is not null and length(trim(acv."Value")) > 0 then 1
                 when acv."Value" is null and lvl."Name" is not null
                      and length(trim(lvl."Name")) > 0 then 1
                 else 0
               end) as "HasValue"
...
summary as (
    select p."InputIdentifier", p."ArticleRevisionID", p."P360Identifier",
           p."Template", p."BusinessCode", p."BusinessName",
           ...
           listagg(case when me."HasValue" = 0 then to_char(me."Characteristic") end, ', ')
             within group (order by to_char(me."Characteristic")) as "CamposFaltantesDetalle"
      from products p
      left join mandatory_evaluation me
        on me."InputIdentifier" = p."InputIdentifier"
     group by p."InputIdentifier", p."ArticleRevisionID", p."P360Identifier",
              p."Template", p."BusinessCode", p."BusinessName"
)
select * from summary order by "InputIdentifier";
