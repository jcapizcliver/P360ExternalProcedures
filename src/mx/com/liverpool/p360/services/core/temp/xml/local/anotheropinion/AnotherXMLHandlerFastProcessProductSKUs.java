...
public class AnotherXMLHandlerFastProcessProductSKUs {
	...
	private java.nio.file.Path normalLogFilePath = java.nio.file.Paths.get("..", "logs", "product_skus_eans_loader_anotheropinion.log");
    ...
    public static void main(String[] args) throws Exception {
    	...
    	AnotherXMLHandlerFastProcessProductSKUs an = new AnotherXMLHandlerFastProcessProductSKUs();
    	...
    	if(args.length < 1) {
			...
			try(java.io.PrintWriter pw = new java.io.PrintWriter(new java.io.OutputStreamWriter( new java.util.zip.GZIPOutputStream( new java.io.FileOutputStream(java.nio.file.Paths.get("/", "u01", "stage", "missing_parents_m2_" + new java.text.SimpleDateFormat("yyyyMMddHHmmssSSS").format(new java.util.Date()), ".csv.gz").toFile()) )))){
				pw.println("ID");
    ...
    private void eleseFinish() {
		...
		log( "" + rw.getRw().makeRequest("POST", "/AAAAZpaMbww/messages", qp, 
				new org.json.JSONObject().put("text", 
						"Envio Express de Productos Finalizado. Productos procesados: " + lacuenta + " gen/ind (" + lacuentaVars + " vars, " + perdidas + " variantes sueltas) 😁.").toString()) );
    }
