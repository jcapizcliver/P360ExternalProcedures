...
public class AnotherXMLHandlerSecondOpinionProducts {
    ...
    public static void main(String[] args) throws Exception {
    	...
    	AnotherXMLHandlerSecondOpinionProducts an = new AnotherXMLHandlerSecondOpinionProducts();
    	...
    	if(args.length < 1) {
    		...
    		t.setDaemon(true);
    		t.start();
			an.runOverDirectories(args[0], elp);
	        elp.processRemaining();
			if(an.rows.length() > 0) {
				an.rw.writeData("list", "Product2G", null, an.qp, an.request, an::log);
			}
			an.rw.writeData("list", "Product2G", "Product2GStructureMap", an.qp, an.requestStructureGroup, System.out::println);
	        ...
	        request.put("columns", an.columns);
	        ...
	        an.log("Ahora los que anduvieron perdidos son: " + an.perdidas);
			try(java.io.PrintWriter pw = new java.io.PrintWriter(new java.io.OutputStreamWriter( new java.util.zip.GZIPOutputStream( new java.io.FileOutputStream(java.nio.file.Paths.get("/", "stage", "missing_parents_m2_" + new java.text.SimpleDateFormat("yyyyMMddHHmmssSSS").format(new java.util.Date()), ".csv.gz").toFile()) )))){
				pw.println("ID");
				an.missingParents.forEach(pw::println);
    ...
    private void runOverDirectories(String startDirectory, LoadProductDataRemainingFieldsOnSpecificProducts elp) throws ParserConfigurationException, SAXException, IOException {
    	log("Running over... " + startDirectory);
    	java.io.File[] files = new java.io.File(startDirectory).listFiles();
    	for(java.io.File f : files) {
    		if(f.isDirectory()) {
    			runOverDirectories(f.getAbsolutePath(), elp);
    		}else {
    			log("\tNow processing " + f.getAbsolutePath());
		    	procesaArchivoYProducto(f.getAbsolutePath());
				elp.process(f.getAbsolutePath());
    		}
    	}
    }
