...
public class CuentaOtrosProductos {
    ...
    public static void main(String[] args) throws Exception {
    	...
    	CuentaOtrosProductos an = new CuentaOtrosProductos();
        ...
        java.io.File[] files = new java.io.File(args[0]).listFiles(ff -> ff.getName().startsWith("Gral"));
        long refProductsCount = 0l;
        long in = System.currentTimeMillis();
        an.log("Now reading files... " + files.length);
        ...
        for(File input : files) {
	        ...
	        try {
		        ...
		        for (Product product : handler.getFinished()) {
	        		an.processProduct(product, requestProduct, requestArticle, an.currentProducts);
		        }
    ...
    private void processProduct(Product product, org.json.JSONObject requestProduct, org.json.JSONObject requestArticle, java.util.List<String> alreadyInSystem) {
    	...
    	if(values != null) {
    		...
    		if(business == null) {
    			log("No se tuvo negocio: " + product.getId() + " - " + n1 + ", " + n2);
    		}else {
	    		Integer freq = cuentaPorNegocio.get(business);
	    		cuentaPorNegocio.put(business, (freq == null ? 0 : freq) + 1);
    		}
    	}
