...
public class CharacteristicMoveAlternativeIdentifiers {
	...
	public static void main(String[] args) throws ServiceUnavailableException {
		...
		for(String extSys : extSystems) {
			...
			columns.put(new org.json.JSONObject().put("identifier", "CharacteristicIdentifier.AlternativeIdentifier(" + extSys + ")"));
			...
			do {
				...
				if(response != null) {
					...
					for(int i=0; i<rows.length(); i++) {
						...
						rowsPayload.put(new org.json.JSONObject().put("object", new org.json.JSONObject().put("id", "'" + values.getString(0) + "'")).put("values", new org.json.JSONArray().put(values.getString(1))));
						if(rowsPayload.length() == 120) {
							rwq.makeRequest("POST", "/list/Characteristic", qp, request.toString());
