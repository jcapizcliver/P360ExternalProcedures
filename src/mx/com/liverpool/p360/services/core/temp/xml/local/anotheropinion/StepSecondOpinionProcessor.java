...
/**
 * Only the DB-dependent "second opinion" / collision-resolution work.
 * Normal STEP field writes live in StepWriterPipeline, so this class does not
 * duplicate Product2G/Article normal payloads anymore.
 */
public final class StepSecondOpinionProcessor {
    ...
    private final RequestHandler reqProductStatus;
    private final RequestHandler reqArticleStatus;
    ...
    public StepSecondOpinionProcessor(
        ...
        reqProductStatus = new RequestHandler(new JSONArray()
                ...
                .put(new JSONObject().put("identifier", "Product2GCharacteristicValueLang.Value('EnriquecidoEnForo',root,\"0000.0000.RK\",'EnriquecidoEnForo',-1)")),
                1000,
                request -> rw.writeData("list", "Product2G", null, qp, request, this::log));

        reqArticleStatus = new RequestHandler(new JSONArray()
    ...
    public void accept(Product product) {
        ...
        if (product.getParentId() != null && product.getParentId().matches("^(S?[0-9]+)")) return;

        Map<String, Value> values = product.getValueMap();
        Value skuValue = values.get("SKU");
        Value firstDateApprove = values.get("FirstDateApprove");
        String sku = text(skuValue);
        String incomingProductId = product.getId();
        String effectiveProductId = incomingProductId;
        String effectiveObjectId = null;
            ...
            if (!incomingAlreadyHasData) {
                if (skuOwner.length() < 15) {
                    ...
                    if (ownerObjectId != null) {
                        ...
                        effectiveObjectId = ownerObjectId;
                    }
                } else {
                    reqAltID.addRow(row("'" + skuOwner + "'@1", new JSONArray().put(incomingProductId)));
                    reqAltID.addRow(row("'" + incomingProductId + "'@1", new JSONArray().put(skuOwner)));
                }
            } else {
                if (skuOwner.length() < 15) {
                    String ownerObjectId = resolveInternalObjectId("Product2G", skuOwner);
                    ...
                    log("Resolviendo combinación de productos: " + skuOwner + " <-> " + incomingProductId);
        ...
        JSONObject statusData = db.productData.get(
                skuOwner != null && !skuOwner.isBlank() && effectiveProductId.equals(incomingProductId)
                        ? skuOwner : effectiveProductId);
        if (statusData == null) statusData = db.productData.get(incomingProductId);

        if (statusData == null || statusData.optString("CurrentStatus", "").isEmpty()) {
            Value calculatedWFAtt = values.get("CalculatedWF_Att");
            Value fotoTomadaLiverpool = values.get("FotoTomadaLiverpool");
            Value stateSKU = values.get("StateSKU");
            String firstApproved = idOrText(firstDateApprove);
            String[] bundle = statusComputer.computeStatus(
                    text(calculatedWFAtt),
                    !firstApproved.isEmpty() ? "Aprobado" : text(stateSKU),
                    text(fotoTomadaLiverpool),
                    incomingProductId);
            currentStatus = bundle[0];
            prevStatus = bundle[1];
            externalStatus = currentStatus == null || currentStatus.isEmpty()
                    ? "" : internalToExternalStatusMap.get(currentStatus);
            String target = effectiveObjectId != null ? effectiveObjectId : "'" + effectiveProductId + "'@1";
            reqProductStatus.addRow(row(target,
                    new JSONArray().put(currentStatus).put(prevStatus).put(externalStatus).put(bundle[2])));
        }

        for (Product child : product.getProducts()) {
            processChild(child, effectiveProductId, currentStatus, prevStatus, externalStatus);
        }
        if (product.getProducts().isEmpty() && !product.getUserTypeId().startsWith("SalesItemFamily")) {
            processChild(product, effectiveProductId, currentStatus, prevStatus, externalStatus);
        }
    }
    ...
    private void processChild(
        ...
        if (skuOwner != null && !skuOwner.isBlank() && !skuOwner.equals(incomingArticleId)) {
            ...
            if (!incomingAlreadyHasData) {
                if (skuOwner.length() < 15) {
                    ...
                    // This is a write operation, not a data lookup; keep the existing API write.
                    ...
                    reqArticleAltID.addRow(row("'" + skuOwner + "'@1", new JSONArray().put(incomingArticleId)));
                    reqArticleAltID.addRow(row("'" + incomingArticleId + "'@1", new JSONArray().put(skuOwner)));
                }
            } else {
                if (skuOwner.length() < 15) {
                    String ownerObjectId = resolveInternalObjectId("Article", skuOwner);
        ...
        if (currentStatus != null && prevStatus != null && externalStatus != null) {
            String target = targetObjectId != null ? targetObjectId : "'" + incomingArticleId + "'@1";
            reqArticleStatus.addRow(row(target,
                    new JSONArray().put(currentStatus).put(prevStatus).put(externalStatus)));
        }
    }
    ...
    private String resolveInternalObjectId(String entity, String identifier) {
        ...
        query.put("items", "'" + identifier + "'@1");
        final String[] objectId = new String[1];
        rw.collectData("list", entity, null, "byItems", query, row -> {
            if (row != null && row.has("object")) {
                objectId[0] = row.getJSONObject("object").optString("id", null);
            }
        });
        return objectId[0];
    }
