...
public class AnotherXMLHandlerSendNames {
    ...
    public static void main(String[] args) throws Exception {
    	...
    	AnotherXMLHandlerSendNames an = new AnotherXMLHandlerSendNames();
        if (args.length == 0) {
    	...
    	an.normalLogFilePath = java.nio.file.Paths.get("..", "logs", args.length == 1 ? "" : args[1], "list_api_load_from_step_sn_.log");
    	an.correctIDsFilePath = java.nio.file.Paths.get("..", "logs", args.length == 1 ? "" : args[1], "list_api_load_from_step_sn_proposals_correct.log");
    	an.errorIDsFilePath = java.nio.file.Paths.get("..", "logs", args.length == 1 ? "" : args[1], "list_api_load_from_step_sn_proposals_wrong.log");
...
//        }
        ...
        an.requestArticle.put("columns", an.columnsArticle);
        an.requestProduct.put("rows", an.rowsProduct);
        an.requestArticle.put("rows", an.rowsArticle);
        an.columnsArticle.put(new org.json.JSONObject().put("identifier", "ArticleCharacteristicValueLang.Value('Name',root,\"0000.0000.RK\",'Name',-1)"));
        an.columnsProduct.put(new org.json.JSONObject().put("identifier", "Product2GCharacteristicValueLang.Value('Name',root,\"0000.0000.RK\",'Name',-1)"));
        an.columnsProduct.put(new org.json.JSONObject().put("identifier", "Product2GCharacteristicValueLang.Value('ProductName',root,\"0000.0000.RK\",'ProductName',-1)"));
...
//        }
        ...
        if(an.rowsArticle.length() > 0) {
        	an.sendData("Article", an.requestArticle);
        }
        if(an.sendLkpValues) {
        ...
        for(File input : files) {
        	if(input.isDirectory()) {
		        ...
		        try {
			        parser.parse(input, handler);
			        for (Product product : handler.getFinished()) {
		        		processProduct(product, requestProduct, requestArticle);
			        }
		        }catch(org.xml.sax.SAXParseException e) {
    ...
    private void processProduct(Product product, org.json.JSONObject requestProduct, org.json.JSONObject requestArticle) {
    	...
    	if(values != null) {
    		...
    		Value value = valMap.get("ProductName");
    		productName = value == null ? null : value.getText();
    		if( (sendProduct && !specific) || 
    			...
    			if( "SalesItemVariant".equals(product.getUserTypeId()) ) {
    				...
    				if(parentId != null) {
		    			addRow("Article", product, productName, requestArticle);
		    			if(rowsA.length() == bs) {
    				...
    				if(parentId != null && !parentId.startsWith("EU") && !parentId.startsWith("UnCatLevel")) {
    					...
    					addRow("Product2G", product, productName, requestProduct);
		    			if(rowsP.length() == bs) {
    ...
    private void addRow(String entity, Product product, String productName, org.json.JSONObject requestProduct) {
    	org.json.JSONArray rowsP = requestProduct.getJSONArray("rows");
		...
		if("Product2G".equals(entity)) {
			rowValues.put( product.getName() == null ? "" : product.getName() );
			rowValues.put( productName == null ? "" : productName );
