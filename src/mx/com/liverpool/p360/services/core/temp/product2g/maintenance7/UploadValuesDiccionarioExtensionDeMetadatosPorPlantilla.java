...
public class UploadValuesDiccionarioExtensionDeMetadatosPorPlantilla {
	...
	public static void main(String[] args) {
		...
		RequestHandler rh = new RequestHandler( new org.json.JSONArray()
			...
			, 1000, request -> rw.writeData("list", "StandardizationValue", null, qp, request, System.out::println) );
		int[] times = new int[] {0};
		SimpleDelimitedFileParser parser = new SimpleDelimitedFileParser( '"', '\t', '\\', "\n", java.nio.charset.StandardCharsets.UTF_8, row -> {
			...
			if(row.length > 0) {
				rh.addRow(new org.json.JSONObject().put("object", new org.json.JSONObject().put("id", "'" + row[0] + "'@'Dos'")).put("values", new org.json.JSONArray().put(row[1]).put(row[2]).put(row[3]).put(row[4]).put(row.length < 6 ? "" : row[5])));
			}
		} );
