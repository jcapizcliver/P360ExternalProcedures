...
public class AnotherXMLHandlerFastProcessRelations {
	...
	private java.nio.file.Path normalLogFilePath = java.nio.file.Paths.get("..", "logs", "sending_relations.log");
    ...
    public static void main(String[] args) throws Exception {
    	...
    	AnotherXMLHandlerFastProcessRelations an = new AnotherXMLHandlerFastProcessRelations();
    	...
    	if(args.length < 1) {
	        ...
	        for(java.util.Map.Entry<String, String> entry : an.childParent.entrySet()) {
	        	...
	        	if(rows.length() == 5000) {
	        		an.rw.writeData("list", "Article", "ProductReference", qp, request, System.out::println);
	        	}
	        }
	        if(rows.length() > 0) {
	        	an.rw.writeData("list", "Article", "ProductReference", qp, request, System.out::println);
    ...
    private void eleseFinish() {
		...
		log( "" + rw.getRw().makeRequest("POST", "/AAAAZpaMbww/messages", qp, 
				new org.json.JSONObject().put("text", 
						"Carga de Relaciones Finalizada. Relaciones procesadas: " + childParent.size() + " 😁.").toString()) );
    }
    ...
    private void processProduct(Product product) {
    	if(product.getParentId().matches("^(S?[0-9]+)$")) {
    		perdidas++;
    		childParent.put(product.getId(), product.getParentId());
    		return;
    	}
    	...
    	if(children != null && !children.isEmpty()) {
    		for(Product child : children) {
    			childParent.put(child.getId(), child.getParentId());
    		}
    	}else {
    		if(!product.getUserTypeId().startsWith("SalesItemFamily"))
    			childParent.put(product.getId(), product.getId());
    	}
    	lacuenta++;
    	if(lacuenta % 10000 == 0) {
    		System.out.print(".");
