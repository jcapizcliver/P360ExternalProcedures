...
public class AnotherXMLHandlerFastProcessListAttributeIDAlongAllThemAll {
	...
	private java.nio.file.Path normalLogFilePath = java.nio.file.Paths.get("..", "logs", "list_attribute_ids_along_themall.log");
    ...
    public static void main(String[] args) throws Exception {
    	...
    	AnotherXMLHandlerFastProcessListAttributeIDAlongAllThemAll an = new AnotherXMLHandlerFastProcessListAttributeIDAlongAllThemAll();
    	...
    	if(args.length < 1) {
			...
			try(java.io.PrintWriter pw = new java.io.PrintWriter(new java.io.OutputStreamWriter(new java.io.FileOutputStream(java.nio.file.Paths.get("ProductLevelAttributeIDs.csv").toFile())))){
				an.attributeIDs.forEach(pw::println);
			}catch(java.io.IOException e) {
				e.printStackTrace();
			}
			try(java.io.PrintWriter pw = new java.io.PrintWriter(new java.io.OutputStreamWriter(new java.io.FileOutputStream(java.nio.file.Paths.get("ChildProductLevelAttributeIDs.csv").toFile())))){
				an.attsChildProd.forEach(pw::println);
			}catch(java.io.IOException e) {
				e.printStackTrace();
			}
    ...
    private void eleseFinish() {
		...
		log( "" + rw.getRw().makeRequest("POST", "/AAAAZpaMbww/messages", qp, 
				new org.json.JSONObject().put("text", 
						"Finalizado listado de atributos them all. Productos procesados: " + lacuenta + " gen/ind (" + lacuentaVars + " vars) 😁.").toString()) );
    }
    ...
    private final java.util.Set<String> attsProd = new java.util.TreeSet<>();
    private final java.util.Set<String> attsChildProd = new java.util.TreeSet<>();
    
    private void processProduct(Product product) {
    	if(product.getParentId().matches("^(S?[0-9]+)$")) {
    		java.util.List<Value> values = product.getValues();
    		for(Value v : values) {
    			attsChildProd.add(v.getAttributeId());
    		}
    		return;
    	}
    	...
    	if(values != null) {
    		for(Value value : values) {
    			attsProd.add(value.getAttributeId());
    		}
    	}
