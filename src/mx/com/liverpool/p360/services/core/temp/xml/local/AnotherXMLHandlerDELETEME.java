...
public class AnotherXMLHandlerDELETEME {
    ...
    public static void main(String[] args) throws Exception {
    	...
    	AnotherXMLHandlerDELETEME an = new AnotherXMLHandlerDELETEME();
...
//        }
        ...
        try(java.io.PrintWriter pw = new java.io.PrintWriter(new java.io.OutputStreamWriter(new java.io.FileOutputStream(java.nio.file.Paths.get("C:", "opt", "LVP", "tmp", "workspace", "stage", "ToSTEP", "plain.csv").toFile())))){
        	pw.println( an.rw.getRw().serializeChunk( new String[] { "ProposalID", "SKU", "MainBarCode" } ) );
        	an.datas.forEach( d -> pw.println(an.rw.getRw().serializeChunk(d)) );
        }catch(java.io.IOException e) {
        	e.printStackTrace();
        }
        /*
    ...
    private void processProduct(Product product, org.json.JSONObject requestProduct, org.json.JSONObject requestArticle) {
    	...
    	if(values != null) {
    		...
    		for(Value value : values) {
    			...
    			if("SKU".equals(value.attributeId)) {
    				data[0] = value.getText();
    			}else if("MainBarCode".equals(value.getAttributeId())) {
    				data[1] = value.getText();
    			}else if("ParentSKU".equals(value.getAttributeId())) {
