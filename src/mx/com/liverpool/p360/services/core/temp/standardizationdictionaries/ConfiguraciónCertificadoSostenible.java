...
public class ConfiguraciónCertificadoSostenible {

	private static final RESTWrapper rw = new RESTWrapper();
	
	public static void main(String[] args) {
		
		java.nio.file.Path p = java.nio.file.Paths.get("C:", "opt", "LVP", "desorden", "PROD", "plantillas", "certificado_sostenible");
		...
		for(String toi : templatesForEsSostenibleVaD) {
			if(templates.contains(toi)) {
				addValues(toi, "CertificadoSostenible", rh);
			}
		}
		rh.sendData();
	}
	...
	private static void addValues(String template, String characteristic, RequestHandler rh) {
		...
		addValue(template, characteristic, "IsEditable", "1", rh);
		addValue(template, characteristic, "DependentValues", "Y", rh);
		addValue(template, characteristic, "DependentAttribute", "EsSostenibleVaD", rh);
	}
