...
public class AnotherXMLHandlerFastProcessGeneraArchivosPorAtributo {
	...
	private java.nio.file.Path normalLogFilePath = java.nio.file.Paths.get("..", "logs", "genera_archivos_por_atributo.log");
    ...
    private java.util.Map<String, java.io.PrintWriter> writersProduct2G = new java.util.HashMap<>();
    private java.util.Map<String, java.io.PrintWriter> writersArticle   = new java.util.HashMap<>();
    
	private java.util.Map<String, String> characteristics = new java.util.HashMap<>();
    
    public static void main(String[] args) throws Exception {
    	long init = System.currentTimeMillis();
    	AnotherXMLHandlerFastProcessGeneraArchivosPorAtributo an = new AnotherXMLHandlerFastProcessGeneraArchivosPorAtributo();
    	...
    	if(args.length < 1) {
    		...
    		an.rw.collectData("list", "Characteristic", null, "bySearch", qp, row -> {
    			org.json.JSONArray values = row.getJSONArray("values");
    			System.out.println(values);
    			an.characteristics.put(values.getString(0), values.getString(1));
    		});
    		
    		for(String att : product2g) {
    			try {
    				an.writersProduct2G.put(att, new java.io.PrintWriter(new java.io.OutputStreamWriter(new java.io.FileOutputStream(java.nio.file.Paths.get("salidasProduct2G", an.escapeForFileName( att ) + ".csv").toFile()))));
    			}catch(java.io.IOException e) {
    				e.printStackTrace();
    			}
    		}
    		for(String att : product2g) {
    			try {
    				an.writersArticle.put(att, new java.io.PrintWriter(new java.io.OutputStreamWriter(new java.io.FileOutputStream(java.nio.file.Paths.get("salidasArticle", an.escapeForFileName ( att ) + ".csv").toFile()))));
    			}catch(java.io.IOException e) {
    				e.printStackTrace();
    			}
    		}
    		...
    		for(java.util.Map.Entry<String, java.io.PrintWriter> entries : an.writersProduct2G.entrySet()) {
    			entries.getValue().close();
    		}
    		for(java.util.Map.Entry<String, java.io.PrintWriter> entries : an.writersArticle.entrySet()) {
    			entries.getValue().close();
    		}
    ...
    private String escapeForFileName(String name) {
    ...
    private void eleseFinish() {
		...
		log( "" + rw.getRw().makeRequest("POST", "/AAAAZpaMbww/messages", qp, 
				new org.json.JSONObject().put("text", 
						"Finalizada la creación de archivos por atributo. Productos procesados: " + lacuenta + " gen/ind (" + lacuentaVars + " vars) 😁.").toString()) );
    }
    ...
    private void processProduct(Product product) {
    	...
    	if(product.getParentId().matches("^(S?[0-9]+)$")) {
    		for(Value value : values) {
    			pw = writersArticle.get(value.getAttributeId());
    			if(pw != null) {
    				dataType = characteristics.get(value.getAttributeId());
    				pw.println( rw.getRw().serializeChunk( new Object[] { product.getId(), "LOOKUP".equals(dataType) ? nvlById(value) : nvl(value) } ) );
    			}
    		}
    		return;
    	}
    	if(values != null) {
    		for(Value value : values) {
    			pw = writersProduct2G.get(value.getAttributeId());
    			if(pw != null) {
    				dataType = characteristics.get(value.getAttributeId());
    				pw.println( rw.getRw().serializeChunk( new Object[] { product.getId(), "LOOKUP".equals(dataType) ? nvlById(value) : nvl(value) } ) );
    			}
    	...
    	java.util.LinkedList<Product> children = product.getProducts();
    ...
    private void processChild(Product child) {
		...
		for(Value value : values) {
			...
			if(pw != null) {
				dataType = characteristics.get(value.getAttributeId());
				pw.println( rw.getRw().serializeChunk( new Object[] { child.getId(), "LOOKUP".equals(dataType) ? nvlById(value) : nvl(value) } ) );
			}
		}
		lacuentaVars++;
    }
