...
private static final RESTWorkshop workshop = new RESTWorkshop();
	...
	public static void main(String[] args) {
		...
		try(java.io.BufferedReader br = new java.io.BufferedReader(new java.io.InputStreamReader(new java.io.FileInputStream("C:\\opt\\LVP\\tmp\\att confi - LOV valor.tsv")))){
			String line = null;
			String[] pieces = null;
			String sep = "\t";
			String delim = "";
			...
			while((line = br.readLine()) != null) {
				...
				if(!"".equals(pieces[2])) {
					rows.put(new org.json.JSONObject().put("object", new org.json.JSONObject().put("id", "'" + pieces[2] +"'@'" + pieces[0] + "'" )).put("values", new org.json.JSONArray().put(pieces[1]).put(true)));
					if(rows.length() == 500) {
						...
						System.out.println(responseJ == null ? workshop.getRawResponse() : responseJ.remove("counters"));
						while(rows.length() > 0) {
							rows.remove(0);
						}
					}
