...
public class AnotherXMLHandlerIndexFileContents {
	...
	private java.nio.file.Path normalLogFilePath = java.nio.file.Paths.get("..", "logs", "index_file_contents.log");
    ...
    public static void main(String[] args) throws Exception {
    	...
    	AnotherXMLHandlerIndexFileContents an = new AnotherXMLHandlerIndexFileContents();
    	...
    	an.normalLogFilePath = java.nio.file.Paths.get("..", "logs", args.length == 1 ? "" : args[1], "index_file_contents.log");
    	...
    	try {
    		refProductsCount = an.processDirectory(args[0]);
    		try(java.io.PrintWriter pw = new java.io.PrintWriter(new java.io.OutputStreamWriter(new java.io.FileOutputStream(java.nio.file.Paths.get(args.length == 1 ? "Index" : args[1]).toFile())))){
	    		for(java.util.Map.Entry<String, java.util.List<String[]>> entry : an.data.entrySet()) {
	    			java.util.List<String> serialized = new java.util.ArrayList<>();
	    			for(String[] pieces : entry.getValue()) {
	    				serialized.add( an.rw.getRw().serializeChunk( pieces, "\"", "|", "\\" ) );
	    			}
	    			pw.println( an.rw.getRw().serializeChunk( new Object[] { entry.getKey(), an.rw.getRw().serializeChunk( serialized.toArray( new String[] {} ), "\"", ";", "\\" ) } ) );
	    		}
        ...
        an.log("Done. (" + refProductsCount + ") " + an.rw.getRw().formatTime(System.currentTimeMillis() - init) );
    }
    
    private final java.util.Map<String, java.util.List<String[]>> data = new java.util.HashMap<>();
    
    private void processProduct(Product product, String filePath) {
    	java.util.List<String[]> innerData = data.get(product.getId());
    	if(innerData == null) {
    		innerData = new java.util.ArrayList<>();
    		data.put(product.getId(), innerData);
    	}
    	innerData.add(new String[] { product.getUserTypeId(), product.getParentId(), filePath });
    	java.util.LinkedList<Product> children = null;
    	children = product.getProducts();
    	if(children != null && !children.isEmpty()) {
    		for(Product child : children) {
    			processProduct(child, filePath);
    		}
    	}
    }
    
    
    private int processDirectory(String dirPathStr) throws ParserConfigurationException, Throwable {
        ...
        java.io.File[] files = new java.io.File(dirPathStr).listFiles();
        int refProductsCount = 0;
        for(File input : files) {
        	if( input.isDirectory() ) {
        		processDirectory(input.getAbsolutePath());
        	}else{
		        Handler handler = new Handler();
		        try {
		        	parser.parse(input, handler);
			        refProductsCount += handler.getPrductsCounter();
			        java.util.List<Product> products = handler.getFinished();
			        for(Product p : products) {
			        	processProduct(p, input.getAbsolutePath());
			        }
