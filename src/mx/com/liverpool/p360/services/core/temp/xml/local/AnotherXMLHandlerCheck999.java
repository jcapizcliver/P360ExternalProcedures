...
public class AnotherXMLHandlerCheck999 {
	...
	private java.nio.file.Path normalLogFilePath = java.nio.file.Paths.get("..", "logs", "list_api_load_from_step_second_opinion_for_relations.log");
    ...
    public static void main(String[] args) throws Exception {
    	...
    	AnotherXMLHandlerCheck999 an = new AnotherXMLHandlerCheck999();
        ...
        an.log("Los values: " + an.losValues.size());
        int am = 0;
        for(java.util.Map.Entry<String, java.util.Set<String>> entry : an.losValues.entrySet()) {
        ...
        an.log("Con un solo producto asociado: " + am);
        an.losValues.entrySet().stream().filter(ent -> ent.getValue().size() > 1).forEach(System.out::println);
    ...
    private final java.util.Map<String, java.util.Set<String>> losValues = new java.util.HashMap<>();
    private java.util.Set<String> losEncontrados = new java.util.TreeSet<>();
    	...
    	for(Value value : values) {
    		if("SKU".equals(value.getAttributeId())) {
    			if(value.getText() != null) {
	    			if(value.getText().startsWith("999")) {
	    				losEncontrados = losValues.get(value.getText());
	    				if(losEncontrados == null) {
	    					losEncontrados = new java.util.TreeSet<>();
	    					losValues.put(value.getText(), losEncontrados);
	    				}
	    				losEncontrados.add(product.getId());
	    			}
    			}else {
    				System.out.println("A va: " + product.getId() + " | " + file);
    			}
    		}
    	}
    	lacuenta++;
