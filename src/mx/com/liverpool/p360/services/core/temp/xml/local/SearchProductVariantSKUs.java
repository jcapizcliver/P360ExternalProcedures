...
public class SearchProductVariantSKUs {
    ...
    public static void main(String[] args) throws Exception {
    	...
    	SearchProductVariantSKUs an = new SearchProductVariantSKUs();
        ...
        an.log("Missings:");
        String[] data = an.collectCurrentlyFirstDateApproved();
        java.util.Map<String, String> qp = new java.util.HashMap<>();
        qp.put("includeObjectsInProtocol", "false");
        RequestHandler rh = new RequestHandler( new org.json.JSONArray().put(new org.json.JSONObject().put("identifier", "Article.FirstDateApproved")), 1000, request -> an.rw.writeData("list", "Article", null, qp, request, an::log) );
        java.time.format.DateTimeFormatter fm = java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss.SSS").withZone(java.time.ZoneId.systemDefault());
        String elval = null;
        try(
        	java.io.PrintWriter pw  = new java.io.PrintWriter( new java.io.OutputStreamWriter(new java.io.FileOutputStream(java.nio.file.Paths.get("SKUVariantFound").toFile())) );
        	java.io.PrintWriter pwN = new java.io.PrintWriter( new java.io.OutputStreamWriter(new java.io.FileOutputStream(java.nio.file.Paths.get("SKUVariantNotFound").toFile())) ) 
        ){
	        for(int i=0; i<an.IDs.length; i++) {
	        	if(java.util.Arrays.binarySearch(those, an.IDs[i]) < 0) {
	        		pwN.println(an.IDs[i]);
	        	}else {
	        		if(java.util.Arrays.binarySearch(data, an.IDs[i]) < 0) {
	        			java.util.Set<java.time.Instant> instants = an.skuEncounters.get(an.IDs[i]);
	        			if(instants != null) {
	        				java.util.List<java.time.Instant> ins = new java.util.ArrayList<>(instants);
	        				elval = an.skuToArtID.get(an.IDs[i]);
	        				an.log("### " + new org.json.JSONObject().put("object", new org.json.JSONObject().put("id", "'" + elval + "'@1")).put("values", new org.json.JSONArray().put( fm.format( ins.get(0) ) )));
//	        				rh.addRow(new org.json.JSONObject().put("object", new org.json.JSONObject().put("id", "'" + an.IDs[i] + "'@1")).put("values", new org.json.JSONArray().put( fm.format( ins.get(0) ) )));
	        			}
	        		}
	        		pw.println(an.IDs[i]);
	        	}
...
//        rh.sendData();
        ...
        an.log("Sin SKU: " + an.sinSKU);
        System.out.println("Sin SKU: " + an.sinSKU);
        an.log("Done. " + an.rw.getRw().formatTime(System.currentTimeMillis() - init) );
    }
    
    private String[] collectCurrentlyFirstDateApproved() {
    	String[] data = null;
    	StringBuilder sb = new StringBuilder();
    	int a = 0;
    	java.util.Map<String, String> qp = new java.util.HashMap<>();
    	qp.put("fields", "Article.SupplierAID,Article.SKU,Article.FirstDateApproved");
    	qp.put("pageSize", "5000");
    	java.util.List<String> skus = new java.util.ArrayList<>();
    	for( String fnd : found ) {
    		sb.append(sb.length() == 0 ? "" : ",").append(fnd);
    		a++;
    		if(a % 1000 == 0) {
    ...
    private final java.util.Map<String, java.util.Set<java.time.Instant>> skuEncounters = new java.util.HashMap<>();
    private final java.util.regex.Pattern p = java.util.regex.Pattern.compile(".+pépele([0-9]+)\\.xml");
    private java.util.regex.Matcher m;
    private final java.util.Map<String, String> skuToArtID = new java.util.HashMap<>();
    ...
    private void loadIDs() {
    	try(java.io.BufferedReader br = new java.io.BufferedReader(new java.io.InputStreamReader(new java.io.FileInputStream(java.nio.file.Paths.get("VariantSKUsToSearch").toFile())))){
    		String line = null;
    		IDs = new String[1024];
    ...
    private int sinSKU = 0;
    
    private void processProduct(Product product) {
    	java.util.LinkedList<Product> childProducts = product.getProducts();
    	if(childProducts == null && "SalesItem".equals(product.getUserTypeId())) {

    		boolean wasThereSKU = false;
        	java.util.LinkedList<Value> values = product.getValues();
        	for(Value value : values) {
        		if("SKU".equals(value.getAttributeId())) {
        			if(value.getText() == null) {
        				System.out.println("WANR Missing SKU: " + value.getAttributeId() + " of " + product.getId() + " in " + currentFile.getAbsolutePath());
        				break;
        			}
        			wasThereSKU = true;
		        	if(java.util.Arrays.binarySearch(IDs, value.getText()) > -1) {
		        		relevantFiles.add(currentFile.getAbsolutePath());
		        		found.add(value.getText());
		        		skuToArtID.put(value.getText(), product.getId());
		        		m = p.matcher(currentFile.getAbsolutePath());
		        		if(!m.find()) {
		        			System.out.println("Archivo sin la norma (" + currentFile.getAbsolutePath() + "). Saliendo");
		        			System.exit(0);
		        		} else {
		        			 String millis = m.group(1);
		        			 java.time.Instant ci = java.time.Instant.ofEpochMilli( Long.parseLong(millis) );
		        			 java.util.Set<java.time.Instant> instants = skuEncounters.get(value.getText());
		        			 if(instants == null) {
		        				 instants = new java.util.TreeSet<>();
		        				 skuEncounters.put(value.getText(), instants);
		        			 }
		        			 instants.add(ci);
		        		}
		        	}
    		...
    		if(!"SalesItem".equals(product.getUserTypeId()) && (childProducts == null || childProducts.isEmpty()) ) {
    			System.out.println("This one is not correct: " + product.getId() + " within " + currentFile.getAbsolutePath());
    		}else {
