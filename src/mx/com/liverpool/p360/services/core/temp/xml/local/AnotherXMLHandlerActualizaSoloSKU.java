...
public class AnotherXMLHandlerActualizaSoloSKU {
    ...
    public static void main(String[] args) throws Exception {
    	...
    	AnotherXMLHandlerActualizaSoloSKU an = new AnotherXMLHandlerActualizaSoloSKU();
...
//        	an.logE(e);
//        }
        try(java.util.stream.Stream<String> lns = java.nio.file.Files.lines(java.nio.file.Paths.get("de_interés"))){
        	lns.forEach(an.aEnviar::add);
        }catch(java.io.IOException e) {
        	an.logE(e);
        }
        ...
        columnsProduct.put(new org.json.JSONObject().put("identifier", "Product2GCharacteristicValueLang.Value('SKU',root,\"0000.0000.RK\",'SKU',-1)"));
        columnsArticle.put(new org.json.JSONObject().put("identifier", "ArticleCharacteristicValueLang.Value('SKU',root,\"0000.0000.RK\",'SKU',-1)"));
        long current = 0l;
    	...
    	if(values != null) {
    		...
    		if( (sendProduct && !specific) || 
    			...
    			if( "SalesItemVariant".equals(product.getUserTypeId()) ) {
    				...
    				if(parentId != null) {
    					addRow("Article", product.getId(), skuValue.getText(), requestArticle);
		    			if(rowsA.length() == bs) {
    				...
    				if(parentId != null && !parentId.startsWith("EU") && !parentId.startsWith("UnCatLevel")) {
    					...
    					addRow("Product2G", product.getId(), skuValue.getText(), requestProduct);
		    			if(rowsP.length() == bs) {
