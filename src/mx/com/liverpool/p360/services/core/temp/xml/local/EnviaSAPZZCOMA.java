...
public class EnviaSAPZZCOMA {
	...
	private java.nio.file.Path normalLogFilePath = java.nio.file.Paths.get("..", "logs", "list_api_load_from_step_second_opinion_sap_zzcoma.log");
    ...
    public static void main(String[] args) throws Exception {
    	...
    	EnviaSAPZZCOMA an = new EnviaSAPZZCOMA();
    	java.nio.file.Files.createDirectories( java.nio.file.Paths.get("..", "logs", args.length == 1 ? "" : args[1]) );
    	an.normalLogFilePath = java.nio.file.Paths.get("..", "logs", args.length == 1 ? "" : args[1], "list_api_load_from_step_second_opinion.log");
    	if (args.length == 0) {
        ...
        String[] betterFit = java.util.Arrays.copyOf(an.zzcoma, an.zzcomaIndex);
        java.util.Arrays.sort(betterFit);
        String[] ostias = new String[an.zzcomaIndex];
        int index = 0;
        String prevOstia = null;
        for(int i = 0; i<betterFit.length; i++) {
        	if(prevOstia != null && !prevOstia.equals(betterFit[i])) {
        		ostias[index] = prevOstia;
        		index++;
        	}
        	prevOstia = betterFit[i];
        }
        ostias[index] = prevOstia;
        index++;
        betterFit = java.util.Arrays.copyOf(ostias, index);
        an.log("Real ostias: " + betterFit.length);
        ...
        for(int i=0; i<betterFit.length; i++) {
        	rows.put(new org.json.JSONObject().put("object", new org.json.JSONObject().put("id", "'" + betterFit[i] + "'@'SAP_ZZCOMA_LOV'")).put("values", new org.json.JSONArray()));
        	if(i % 10000 == 0) {
        		an.rw.writeData("list", "LookupValue", null, qp, request, an::log);
        		while(rows.length() > 0) {
        			rows.remove(0);
        		}
        	}
    ...
    private final String[] zzcoma = new String[4500000];
    private int zzcomaIndex = 0;
    ...
    private void processProduct(Product product) {
    	...
    	if(values != null) {
    		...
    		if(!"".equals(sapZzcomaStr)) {
    			zzcoma[zzcomaIndex] = sapZzcomaStr;
    			zzcomaIndex++;
    		}
    	}
    	lacuenta++;
