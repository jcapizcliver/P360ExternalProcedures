...
public class AnotherXMLHandlerSendRemaining {
...
//	private java.util.regex.Pattern idPattern = java.util.regex.Pattern.compile("'(S[0-9]+)'@1");
...
//        java.math.BigDecimal HUNDRED = java.math.BigDecimal.TEN.multiply(java.math.BigDecimal.TEN);
        long current = 0l;
        java.util.ArrayList<String> toBeProcessed = new java.util.ArrayList<>();
        an.readToBeReSent(toBeProcessed);
        an.log("Working on: " + toBeProcessed.size());
        for(File input : files) {
	        Handler handler = an.new Handler();
	        parser.parse(input, handler);
	        for (Product product : handler.getFinished()) {
	        	if(toBeProcessed.contains(product.getId())) {
...
//	        		current ++;
	        		...
	        		for(Product cp : childProducts) {
	        			if(toBeProcessed.contains(cp.getId())) {
	        				an.processProduct(cp, requestProduct, requestArticle);
	        			}
	        		}
	        	}
	        }
