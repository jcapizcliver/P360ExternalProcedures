...
public class LoadProductDataSecondOpinionForFlatAttributeValuess {
    ...
    public static void main(String[] args) throws Exception {
    	...
    	LoadProductDataSecondOpinionForFlatAttributeValuess an = new LoadProductDataSecondOpinionForFlatAttributeValuess();
    	if (args.length == 0) {
        ...
        java.nio.file.Path path = java.nio.file.Paths.get(args.length == 1 ? "ProductTemplateAttributesPerFileValuess.csv" : args[1]);
        try{
    ...
    private void processProduct(Product product, String file) {
    	...
    	if(values != null) {
    		...
    		for(Value value : values) {
    			pw.println( rw.getRw().serializeChunk( new Object[] { product.getId(), product.getParentId(), file, value.getAttributeId(), nvl( value.getId() ), nvl( value.getText() )  } ) );
    		}
    	}else {
