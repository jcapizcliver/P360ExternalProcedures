...
//				String response = null;
				for(int i=0; i<rws.length(); i++) {
					...
					if(r.has("sku")) {
						...
						if(proposalId == null) {
							...
							if(resp != null) {
								try {
									org.json.JSONObject jr = new org.json.JSONObject(resp);
									org.json.JSONArray items = jr.getJSONArray("items");
									proposalId = items.getJSONObject(0).getString("product");
									if("".equals(proposalId)) {
										proposalId = null;
									}
								}catch(org.json.JSONException e) {
						...
						if(proposalId == null) {
							if(r.has("proposalId")) {
								resp = dr.getProductByVariant(new org.json.JSONArray().put(r.getString("proposalId")));
								if(resp != null) {
									try{
										org.json.JSONObject jr = new org.json.JSONObject(resp);
										org.json.JSONArray items = jr.getJSONArray("items");
						...
						if(proposalId == null) {
							resp = dr.getProductData(new org.json.JSONArray().put(r.getString("proposalId")));
							if(resp != null) {
								try {
									org.json.JSONObject jr = new org.json.JSONObject(resp);
									org.json.JSONArray items = jr.getJSONArray("items");
									...
									if(!"".equals(item.getString("CurrentStatus"))) {
										proposalId = r.getString("proposalId");
									}
								}catch(org.json.JSONException e) {
									logE(e);
								}
...
//			System.out.println();
		...
		try {
			...
			try {
				...
				if(rows != null && !org.json.JSONObject.NULL.equals(rows) && rows.length() > 0) {
					r = rows.getJSONObject(0).getJSONArray("values").getString(0);
					if(isArticle) {
						rawResponse = rc.getRequest("GET", baseUrl + "/object/Article/'" + r + "'@'MASTER'?entityFilter=ProductReference", null);
						response = new org.json.JSONObject(rawResponse);
						r = response.getJSONObject("_data").getJSONArray("higherLevelProduct").getJSONObject(0).getJSONObject("_qualification").getString("referencedIdentifier");
					}
				}
			} catch (IOException e) {
				e.printStackTrace();
				...
				try {
					...
					if(rows != null && !org.json.JSONObject.NULL.equals(rows) && rows.length() > 0) {
						for(int i=0; i<rows.length(); i++) {
							...
							newValues.put(values.getString(0));
							newValues.put("");
							newValues.put("");
							for(int j=1; j<values.length(); j++) {
								...
								epa = j;
								o = values.get(j);
								newValues.put( o instanceof org.json.JSONArray ? values.getJSONArray(j).getString(0) : String.valueOf(o));
							}
							if( (rowComplete/(values.length() - 2)) >= 0.7 ) {
								r.put(newValues);
								variantValues = collectArticleLevelInformation(newValues.getString(0), newValues.getString(6), values);
	...
	private org.json.JSONArray collectData(java.util.Set<String> externalIds) {
		...
		externalIds.forEach(items::put);
		externalIds.forEach(items0::put);
		...
		if(resp != null) {
			try{
				...
				for(int i=0; i<items.length(); i++) {
					newValues = new org.json.JSONArray();
					log("Over: " + items0.getString(i));
					item = items.getJSONObject(i);
					item2 = items2.getJSONObject(i);
					newValues.put(item.getString("product"));
					newValues.put("");
					newValues.put("");
...
//					.put("SupplierPartNumber", values.length > 15 ? values[15] : "")
					...
					newValues.put("");
					r.put(newValues);
					java.util.Set<String> varIds = dr.getVariants(item.getString("product"));
...
//									,item.getString("DescriptionLong2")
								newValuesVariant.put(i0.getString("variant"));
								newValuesVariant.put(item.getString("SKU"));
								newValuesVariant.put(item.get("SupplierID"));
								newValuesVariant.put(item2.getString("supplierShopId"));
								newValuesVariant.put(item.get("Business"));
								newValuesVariant.put(item.get("SAPObjectType"));
								newValuesVariant.put(i0.get("SKU"));
								newValuesVariant.put( deriveName( item2.getString("ProductName"), i0.getString("TamanoUnico"), i0.getString("ColoursLiverpoolAtt") ));
								newValuesVariant.put(i0.get("MainBarCode"));
								newValuesVariant.put(i0.get("MainBarCodeS4H"));
								newValuesVariant.put(item2.getString("BuyerRejectionMessage"));
								newValuesVariant.put(item2.getString("SupplierRejectionMessage"));
								newValuesVariant.put(item.get("ItemGroup"));
								newValuesVariant.put(item.get("Section"));
								newValuesVariant.put(item.get("CurrentStatus"));
								newValuesVariant.put(item2.getString("SkuType"));
								newValuesVariant.put(item2.getString("BWSCL"));
								...
								newValuesVariant.put(item2.getString("TextoAdicional"));
								newValuesVariant.put(item2.get("Evento"));
								newValuesVariant.put(i1.get("CostobrutoSinIVA"));
								newValuesVariant.put(i1.get("PrecioSugeridocIVA"));
								...
								newValuesVariant.put(i0.getString("ColoursLiverpoolAtt"));
								newValuesVariant.put(i0.getString("TamanoUnico"));
